...
exports.addOneLink = (req, res, next) => {
  let key = req.headers.fapp + ":links";
  ...
  redis.get(key).then((data) => {
    ...
    if (
      data.filter((item) => item.name == name && item.src == src).length > 0
    ) {
      return res.json(util.getReturnData(1, "添加失败，已有重复的友链"));
    }
    let tKeylinks = crypto
      .createHash("md5")
      .update(JSON.stringify(`${src}+${name}`))
      .digest("hex");
    let links = {
      src: src,
      name: name,
    ...
    console.log(data);
    redis.set(key, data);
    res.json(util.getReturnData(0, "添加成功"));
  });
};

exports.editOneLink = (req, res, next) => {
  let key = req.headers.fapp + ":links";
  let src = req.body.src;
  let name = req.body.name;
  let id = req.body.id;
  if (!id)
    return res.json(util.getReturnData(1, "友链id未查询到，请检查刷新后重试"));
  redis.get(key).then((data) => {
    const index = data.findIndex((item) => item.id === id);
    if (index !== -1) {
      let links = data[index];
      links = {
        ...links,
        name: name || links.name,
        src: src || links.src,
      };
      data.splice(index, 1, links);
      redis.set(key, data);
      res.json(util.getReturnData(0, "修改成功"));
    } else {
      res.json(util.getReturnData(1, "友链id未查询到，请检查刷新后重试"));
    }
  });
};

exports.deleteOneLink = (req, res, next) => {
  let key = req.headers.fapp + ":links";
  let id = req.body.id;
  if (!id)
    return res.json(util.getReturnData(1, "友链id未查询到，请检查刷新后重试"));
  redis.get(key).then((data) => {
    ...
    if (index !== -1) {
      data.splice(index, 1);
      redis.set(key, data);
      res.json(util.getReturnData(0, "删除成功"));
    } else {
      res.json(util.getReturnData(1, "友链id未查询到，请检查刷新后重试"));
    }
  });
...
exports.stopLogin = (req, res) => {
  ...
  console.log("key", key);
  console.log("alluser", redis.get(key));
